...
namespace PA5_Draft
{
    ...
    class SnakeGame
    {
        internal const int AppleSize = 20;//initalized apple size 
        internal event EatApple EatAndGrow;//triggered when the snake reaches at the location of an apple
        internal event HitAnObstacle HitWallAndLose;//triggered when the snake touches an obstacle
        internal event HitItself HitSnakeAndLose;//triggered when the snake collides with its own body
        private Point Start { get; }
        internal List<LineSeg> SnakeBody { get; }//list of line segments building the snake body
        internal List<LineSeg> Obstacles { get; }//list of line segments including the borders of playing field that the snake must not touch
        internal List<Point> Apples { get; }//list of locations/points in which apples are located
